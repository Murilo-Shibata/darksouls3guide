...
const BossCard = ({ boss }: { boss: typeof bosses[0] }) => {
  const { isDamaged, triggerDamage } = useDamage();

  return (
    <div
      onClick={triggerDamage}
      className={`glass-card rounded-lg overflow-hidden border border-ash hover:border-ember/50 transition-colors group cursor-crosshair select-none ${isDamaged ? 'damage-flash border-red-500' : ''}`}
    >
